...
function read ($conn){
    extract($_POST);
    $query = "CALL attendance_read_sp('$attendanceID')";
...
function update ($conn){
    extract($_POST);
    $query = "CALL attendance_sp('$attendanceID','$stdID','$classID','$courseID','$status','$date','$action')";
    $result = $conn->query($query);

    if($result){
            $result_data = array("status" => true, "message"=>"Attendance has been updated successfully.");
       }
    else{
        $result_data = array("status" => false, "message"=>$conn->error());
    }
